...
<body class="min-h-full font-sans antialiased text-slate-800 bg-[#F8FAFC] flex flex-col justify-center py-8 sm:py-12 px-6 sm:px-8 lg:px-8"
    ...
    <div class="mt-6 w-full max-w-md mx-auto">
        <div class="bg-white py-6 px-6 sm:px-8 shadow-sm border border-slate-200 rounded-2xl space-y-4">
            ...
            <!-- Interactive Error Alert Box (Firebase / Validation) -->
                ...
                <div class="p-3.5 bg-amber-50/90 border border-amber-200 text-amber-900 text-xs rounded-xl shadow-2xs flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-0.5 text-[11.5px] leading-relaxed text-amber-800">
                            Isi <code>FIREBASE_WEB_*</code> di <code>.env</code> untuk mengaktifkan pendaftaran.
                        </div>
                    </div>
                </div>
            @endunless
            ...
            <form @submit.prevent="signUpWithEmail()" class="space-y-3.5">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="name" x-model="name" @input="clearFieldErrors()" name="name" type="text" required
                               placeholder="Nama lengkap"
                               class="w-full bg-slate-50 focus:bg-white border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-brand-500 transition">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="email" x-model="email" @input="clearFieldErrors()" name="email" type="email" autocomplete="email" required
                               ...
                               :class="errorType === 'email-in-use' || errorType === 'invalid-email' ? 'border-rose-400 bg-rose-50/30 focus:border-rose-500 ring-1 ring-rose-400/20' : 'border-slate-200 bg-slate-50 focus:bg-white focus:border-brand-500'"
                               class="w-full rounded-lg pl-9 pr-3 py-2 text-xs text-slate-800 focus:outline-none transition">
                    </div>
                    <p x-show="errorType === 'email-in-use'" x-cloak class="text-[11px] text-rose-600 font-medium mt-1 flex items-center gap-1">
                        <i class="fa-solid fa-circle-xmark text-[10px]"></i> Email sudah terdaftar. <a href="{{ route('login') }}" class="underline font-bold text-brand-700">Masuk</a>
                    </p>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="password" x-model="password" @input="clearFieldErrors()" name="password" type="password" required minlength="6"
                               placeholder="Minimal 6 karakter"
                               :class="errorType === 'weak-password' || errorType === 'password-mismatch' ? 'border-rose-400 bg-rose-50/30 focus:border-rose-500 ring-1 ring-rose-400/20' : 'border-slate-200 bg-slate-50 focus:bg-white focus:border-brand-500'"
                               class="w-full rounded-lg pl-9 pr-3 py-2 text-xs text-slate-800 focus:outline-none transition">
                    </div>
                    <p x-show="errorType === 'weak-password'" x-cloak class="text-[11px] text-rose-600 font-medium mt-1 flex items-center gap-1">
                        <i class="fa-solid fa-circle-xmark text-[10px]"></i> Minimal 6 karakter.
                    </p>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="password_confirmation" x-model="passwordConfirmation" @input="clearFieldErrors()" name="password_confirmation" type="password" required minlength="6"
                               placeholder="Ulangi kata sandi"
                               :class="errorType === 'password-mismatch' ? 'border-rose-400 bg-rose-50/30 focus:border-rose-500 ring-1 ring-rose-400/20' : 'border-slate-200 bg-slate-50 focus:bg-white focus:border-brand-500'"
                               class="w-full rounded-lg pl-9 pr-3 py-2 text-xs text-slate-800 focus:outline-none transition">
                    </div>
                    <p x-show="errorType === 'password-mismatch'" x-cloak class="text-[11px] text-rose-600 font-medium mt-1 flex items-center gap-1">
                        <i class="fa-solid fa-circle-xmark text-[10px]"></i> Konfirmasi sandi tidak sesuai.
                    </p>
                </div>
            ...
            <div class="text-center pt-1 text-xs text-slate-600 border-t border-slate-100">
                Sudah punya akun?
                <a href="{{ route('login') }}" class="font-bold text-brand-600 hover:text-brand-700">Masuk</a>
            </div>
    ...
    <script>
        ...
        function parseAuthError(error) {
            ...
            let title = 'Peringatan Pendaftaran';
            let message = 'Terjadi kesalahan saat mendaftarkan akun.';
            let type = 'general';

            switch (code) {
                case 'auth/email-already-in-use':
                    title = 'Email Sudah Terdaftar';
                    message = 'Alamat email ini sudah terhubung dengan akun lain. Silakan masuk ke akun Anda.';
                    type = 'email-in-use';
                    break;
                case 'auth/weak-password':
                    title = 'Kata Sandi Terlalu Lemah';
                    message = 'Kata sandi terlalu pendek. Gunakan minimal 6 karakter kombinasi huruf dan angka.';
                    type = 'weak-password';
                    break;
                ...
                case 'auth/network-request-failed':
                    ...
                    message = 'Gagal terhubung ke server pendaftaran. Periksa koneksi internet Anda.';
                    type = 'general';
                    break;
                case 'auth/popup-closed-by-user':
                    title = 'Pendaftaran Google Dibatalkan';
                    message = 'Jendela pop-up Google ditutup sebelum proses selesai.';
                    type = 'general';
                    break;
                default:
                    title = 'Pendaftaran Gagal';
                    message = error?.message || 'Terjadi kesalahan saat memproses data akun.';
                    type = 'general';
            }
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('registerPage', () => ({
                ...
                clearFieldErrors() {
                    if (this.errorType === 'weak-password' || this.errorType === 'password-mismatch' || this.errorType === 'email-in-use' || this.errorType === 'invalid-email') {
                        this.errorType = null;
                    }
                },
                ...
                async signUpWithEmail() {
                    ...
                    if (this.password !== this.passwordConfirmation) {
                        this.triggerError({
                            ...
                            message: 'Konfirmasi kata sandi tidak cocok dengan kata sandi yang Anda buat.',
