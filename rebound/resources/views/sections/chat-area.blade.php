...
id: Area utama percakapan dengan AI. Menampilkan pesan AI, pesan user, indikator mengetik, kartu telemetri/rekomendasi,
dan floating chat composer.
Pesan baru dikirim via `sendMessage()`. Riwayat percakapan harus disimpan di database tabel `messages`.
en: Main AI conversation area. Displays AI messages, user messages, typing indicators, telemetry/recommendation cards,
and floating chat composer.
...
<div class="flex-1 flex flex-col h-full max-w-[640px] w-full mx-auto relative overflow-hidden">
    ...
    <div id="chat-messages-container"
        class="flex-1 overflow-y-auto px-1 sm:px-2 pt-1 space-y-4 pb-48 sm:pb-52 custom-scrollbar">
        ...
        <template x-if="!messages || messages.length === 0">
            <div
                class="py-6 px-4 text-center space-y-2 bg-white/80 backdrop-blur-xs border border-slate-200/90 rounded-xl my-4 select-none max-w-sm mx-auto shadow-2xs">
                <div
                    class="w-10 h-10 rounded-full bg-brand-50 text-brand-600 flex items-center justify-center mx-auto text-base">
                    <i class="fa-solid fa-wand-magic-sparkles"></i>
                </div>
                <div class="text-xs font-bold text-slate-800"
                    x-text="lang === 'id' ? 'Sesi Obrolan Baru' : 'Fresh Chat Session'"></div>
                <p class="text-[11px] text-slate-500 leading-relaxed"
                    x-text="lang === 'id' ? 'Riwayat chat sebelumnya telah dibersihkan. Silakan ketik pesan atau pilih saran prompt di bawah untuk memulai analisis percakapan baru.' : 'Previous chat history cleared. Type a message or pick a suggestion chip below to start a new chat.'">
                </p>
            </div>
        </template>
            ...
            <div class="w-full">
                ...
                <template x-if="msg.sender === 'ai'">
                    <div class="flex items-start gap-2 sm:gap-2.5">
                        <!-- AI Bot Icon Avatar -->
                        <div
                            class="w-6 h-6 rounded-md bg-blue-100 text-brand-600 flex items-center justify-center shrink-0 mt-0.5 shadow-2xs text-[10px]">
                        ...
                        <div class="flex-1 max-w-[500px]">
                            <div
                                class="bg-white rounded-lg rounded-tl-none border border-slate-200 p-2.5 sm:p-3 text-xs text-slate-700 leading-relaxed shadow-xs">
                                ...
                                {{-- id: Tampilkan Kartu Rekomendasi Penerbangan jika tertunda & belum rebooked (Figma
                                Node 21:894)
                                ...
                                <template
                                    x-if="(msg.showRecommendation || msg.type === 'options_list') && (msg.type === 'options_list' || flightStatus === 'delayed')">
                                ...
                                <template x-if="msg.showSuccess || msg.type === 'success_card'">
                                    @include('sections.success-card')
                                </template>

                            </div>

                            <span class="text-[9.5px] text-slate-400 font-medium mt-1 ml-1 block"
                ...
                <template x-if="msg.sender === 'user'">
                    <div class="flex items-start justify-end gap-2 sm:gap-2.5">
                        <div class="max-w-[420px]">
                            <div
                                class="bg-brand-600 text-white rounded-lg rounded-tr-none p-2.5 sm:p-3 text-xs leading-relaxed shadow-xs">
                                <p x-text="lang === 'id' ? msg.textId : msg.textEn"></p>
                            </div>
                            <span class="text-[9.5px] text-slate-400 font-medium mt-1 mr-1 text-right block"
                                x-text="msg.time"></span>
                        </div>
        ...
        <div x-show="isTyping" x-cloak class="flex items-center gap-2">
            <div
                class="w-6 h-6 rounded-md bg-blue-100 text-brand-600 flex items-center justify-center shrink-0 text-[10px]">
                <i class="fa-solid fa-robot text-[10px]"></i>
            </div>
            <div class="bg-white border border-slate-200 rounded-lg p-2.5 flex items-center gap-1 shadow-xs">
                ...
                <span class="w-1.5 h-1.5 rounded-full bg-brand-500 animate-bounce"
    ...
    <div
        class="absolute bottom-0 left-0 right-0 z-20 pointer-events-none pb-2 sm:pb-3 pt-4 bg-gradient-to-t from-[#F8FAFC] via-[#F8FAFC]/95 to-transparent">
        ...
        <div class="pointer-events-auto max-w-[620px] w-full mx-auto space-y-1.5">
            ...
            <div class="space-y-1" x-show="dynamicSuggestions && dynamicSuggestions.length > 0">
                <template x-for="(sug, index) in dynamicSuggestions.slice(0, 2)" :key="index">
                    <button @click="sendMessage(lang === 'id' ? sug.id : sug.en)"
                        ...
                        <i
                            class="fa-solid fa-chevron-right text-[8.5px] text-slate-300 group-hover:text-brand-500 transition shrink-0"></i>
                    </button>
                </template>
            </div>
            ...
            <form @submit.prevent="sendMessage()"
                ...
                <button type="submit" :disabled="!chatInput || chatInput.trim() === ''"
